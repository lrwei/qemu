...
static void do_tb_phys_invalidate(TranslationBlock *tb, bool rm_from_page_list)
{
    ...
    if (!(tb_cflags(tb) & CF_BAILOUT)) {
         ...
         * would it be linked to slot other than INFO->LABEL_PTR, nor could it
                                              ...
                                              TCGBailoutInfo *bailout_info)
{
 ...
 error_return:
    ...
    uintptr_t end_of_data = (uintptr_t) tb->tc.ptr + tb->tc.size + search_size;

    /* Up to now, BAILOUT_INFO has been a temporal local storage allocated
     * on the stack, commit cache allocation only after TB has reached its
     * completion.  */
    if (tb->cflags & CF_BAILOUT) {
        size_t off = ROUND_UP(end_of_data, 8) - (size_t) tb->tc.ptr;

        tb->bailout_info_offset = off;
        /* Assert that we do not overflow our stored offset.  */
        g_assert(tb->bailout_info_offset == off);
        /* Search data is guaranteed to below code_gen_highwater, and
         * TCGBailoutInfo itself can't possibly overflow it.  */
        end_of_data = (uintptr_t) (tb_bailout_info(tb) + 1);
        memcpy(tb_bailout_info(tb), bailout_info, sizeof(TCGBailoutInfo));
    }
    qatomic_set(&tcg_ctx->code_gen_ptr, (void *) ROUND_UP(end_of_data,
                                                          CODE_GEN_ALIGN));
