...
#define CF_MONOLITHIC  0x00010000 /* Access guest memory with qemu_{ld, st} */
...
#define TB_JMP_RESET_OFFSET_INVALID 0xffff /* indicates no jump generated */
    ...
    /* Hold offset to BAILOUT_INFO for CF_BAILOUT TBs, allocation made within
     * the code cache, and after TB search data.  */
    uint16_t bailout_info_offset;
};

typedef struct TCGBailoutInfo {
...
static inline TCGBailoutInfo *tb_bailout_info(const TranslationBlock *tb)
{
    tcg_debug_assert(tb_cflags(tb) & CF_BAILOUT);
    return tb->tc.ptr + tb->bailout_info_offset;
}
